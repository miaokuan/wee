...
class Readability
{
    const READABILITY_VERSION = '0.21';
    ...
    private function removeJunkAttr($RootNode, $Attr)
    {
        ...
        while ($Tag = $Tags->item($i++)) {
            ...
            // $this->removeAllAttributes($Tag);
        }

        return $RootNode;
    }

    public function removeAllAttributes(&$Tag)
    {
        // $length = $Tag->attributes->length;
        // for ($i = 0; $i < $length; ++$i) {
        //     $name = $Tag->attributes->item($i)->name;
        //     $Tag->removeAttribute($name)
        // }

        foreach ($Tag->attributes as $name => $attrNode) {
            if (in_array($name, $this->allowAttrs)) {
                continue;
            }
            $Tag->removeAttribute($name);
        }
    ...
    {
        $split_point = '/( \- )|( \| )|( « )/';
        $titleNodes = $this->DOM->getElementsByTagName("title"); //h1
        ...
        if ($titleNodes->length
            ...
            $result = array_map('strrev', preg_split($split_point, strrev($title)));
